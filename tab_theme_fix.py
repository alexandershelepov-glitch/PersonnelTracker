...
def install_tab_theme_fix(window: Any) -> None:
    ...
    def style_tabs(tabs: QTabWidget, owner=None) -> None:
        theme_owner = owner if owner is not None else window
        palette = theme_owner.theme_manager.palette()
    ...
    if profile_class is not None and not getattr(profile_class, "_v083_tab_theme_fixed", False):
        ...
        def profile_init(dialog, *args, **kwargs):
            ...
            owner = dialog.parent()
            if owner is None or not hasattr(owner, "theme_manager"):
                owner = window
            for tabs in dialog.findChildren(QTabWidget):
                style_tabs(tabs, owner)
