...
def install_composition_ui(window: Any) -> None:
    ...
    filters.addWidget(reset_filters, 2, 3)
    ...
    table.setToolTip("Дважды нажмите строку, чтобы открыть профиль работника.")
    ...
    empty_state.hide()
    directory_root.addWidget(empty_state)
    ...
    window.composition_filter_status = filter_status
    window.composition_reset_filters = reset_filters
    window.composition_empty_state = empty_state
    ...
    def filters_active() -> bool:
        return bool(search.text().strip()) or not active_only.isChecked() or any(
            combo.currentText() != "Все"
            for combo in (department, section, group, position)
        )
