...
def install_planning_ui(window: Any) -> None:
    ...
    for index, (caption, combo) in enumerate((
        ...
        filter_row.addWidget(QLabel(caption), row, pair * 2)
        filter_row.addWidget(combo, row, pair * 2 + 1)
    ...
    graph_empty = QLabel("По заданным фильтрам сотрудники не найдены")
    graph_empty.setObjectName("secondaryText")
    graph_empty.setAlignment(Qt.AlignCenter)
    graph_empty.hide()
    graph_root.addWidget(graph_empty)
    ...
    # Keep employee identity visible while leaving a useful calendar viewport
    # at the supported 760 px minimum window width.
    ...
    saved_splitter = window.settings.value("planning/splitter_state")
    if saved_splitter:
        splitter.restoreState(saved_splitter)
    splitter.splitterMoved.connect(
        lambda _position, _index: window.settings.setValue(
            "planning/splitter_state", splitter.saveState()
        )
    )
    ...
    list_empty = QLabel("Событий за выбранный период нет")
    list_empty.setObjectName("secondaryText")
    list_empty.setAlignment(Qt.AlignCenter)
    list_empty.hide()
    list_root.addWidget(list_empty)
    ...
    event_list.setToolTip("Дважды нажмите событие, чтобы открыть его.")
    ...
    window.planning_splitter = splitter
    window.planning_graph_empty = graph_empty
    window.planning_list_empty = list_empty

    filter_status = QLabel()
    filter_status.setObjectName("secondaryText")
    reset_filters = QPushButton("Сбросить фильтры")
    filter_row.addWidget(filter_status, 2, 0, 1, 2)
    filter_row.addWidget(reset_filters, 2, 3)
    window.planning_filter_status = filter_status
    window.planning_reset_filters = reset_filters
    ...
    def month_title(value: QDate) -> str:
        month = ru_locale.standaloneMonthName(value.month(), QLocale.LongFormat)
        if month:
            month = month[0].upper() + month[1:]
        return f"{month} {value.year()}"

    def filters_active() -> bool:
        return any(combo.currentText() != "Все" for combo in (department, section, event_type))
    ...
    def refresh_graph() -> None:
        ...
        graph_empty.setVisible(not persons)
        splitter.setVisible(bool(persons))
        ...
        for row, person in enumerate(persons):
            ...
            for column, value in enumerate(values):
                ...
                if column == 0:
                    ...
                    item.setToolTip(f"{value}\nНажмите, чтобы добавить событие работнику.")
                else:
    ...
    def refresh_list() -> None:
        ...
        list_empty.setVisible(not rows)
        event_list.setVisible(bool(rows))
    ...
    for combo in (department, section, event_type):
        combo.currentTextChanged.connect(
            lambda _text: (refresh_graph(), refresh_list(), update_filter_status())
        )
