...
def install_manual_team_ui(window: Any) -> None:
    ...
    # -------------------- Manual mode -------------------------------------
    manual = QScrollArea()
    manual.setWidgetResizable(True)
    manual.setFrameShape(QFrame.NoFrame)
    manual_body = QWidget()
    manual.setWidget(manual_body)
    manual_root = QVBoxLayout(manual_body)
    manual_root.setContentsMargins(8, 10, 8, 8)
    manual_root.setSpacing(10)
    manual_root.setSizeConstraint(QLayout.SetMinimumSize)
    ...
    actions.addWidget(counter, 0, 0)
    actions.addWidget(copy_mode, 0, 1)
    actions.addWidget(copy_button, 0, 2)
    actions.addWidget(clear_button, 1, 1)
    actions.addWidget(create_button, 1, 2)
    ...
    table.setToolTip("Дважды нажмите строку, чтобы отметить или снять выбор работника.")
    ...
    empty_state = QLabel("По заданным фильтрам сотрудники не найдены")
    empty_state.setObjectName("secondaryText")
    empty_state.setAlignment(Qt.AlignCenter)
    empty_state.hide()
    manual_root.addWidget(empty_state)
    ...
    window.manual_team_filter_status = filter_status
    window.manual_team_reset_filters = reset_filters
    window.manual_team_empty_state = empty_state
    ...
    def filters_active() -> bool:
        return bool(search.text().strip()) or any(
    ...
    def rebuild_table(*_args) -> None:
        ...
        empty_state.setVisible(table.rowCount() == 0)
