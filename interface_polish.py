    ...
    def current_search():
        ...
        if page_index == 0:
            ...
            if (
                ...
                return getattr(window, "manual_team_search", None)
        if page_index == 2:
            tabs = getattr(window, "planning_tabs", None)
            if tabs is not None and tabs.currentIndex() == 1:
                return getattr(window, "planning_search", None)
        return None

    def focus_current_search() -> None:
        search = current_search()
        if search is not None and search.isVisible():
            search.setFocus(Qt.ShortcutFocusReason)
            search.selectAll()

    find_shortcut = QShortcut(QKeySequence.Find, window)
    find_shortcut.activated.connect(focus_current_search)
    back_shortcut = QShortcut(QKeySequence(Qt.Key_Escape), window)
    back_shortcut.activated.connect(
        lambda: window.navigate_back()
        if getattr(window, "_navigation_stack", [])
        and callable(getattr(window, "navigate_back", None))
        else None
    )
    window._interface_find_shortcut = find_shortcut
    window._interface_back_shortcut = back_shortcut
